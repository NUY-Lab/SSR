...
class FileManager:
    ...
    def set_file(self, filepath=None):
        if filepath is None:
            self.__fileIO = FileManager.FileIO(
                filepath=ask_save_filename(
                    filetypes=[
                        ("TEXT", ".txt"),
                    ],
                    defaultextension="txt",
                    initialdir=USER_VARIABLES.DATADIR,
        ...
        # ファイル名はクリップボードにコピーしておく
        pyperclip.copy(os.path.basename(self.__fileIO.filepath))

    def save(self, *args: tuple | str) -> None:
        """データ保存"""
...
class PlotAgency:
    ...
    def run_plot_window(self) -> None:
        ...
        self.share_list = Manager().list()
        self.__isfinish = Value("i", 0)
