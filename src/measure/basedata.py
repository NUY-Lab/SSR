...
class BaseData:
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        if name in self.__class__.__dict__.get("__annotations__").keys():
            super().__setattr__(name, value)
        else:
            raise BaseDataError(
                f"{name}は{self.__class__.__name__}に最初に定義された変数に含まれていません。\n"
    ...
    def __init_subclass__(cls, **kwargs) -> None:
        ...
        if "__init__" not in cls.__dict__.keys():
            ...
            """
            # 文字列で書いたコードを実行
            exec(textwrap.dedent(init_text))
