...
def redefinition_check(mod:ModuleType):
    ...
    dic=dict(filter(lambda item : not re.match("__.*__",item[0]),mod.__dict__.items())) #グローバル空間の変数名のうち、自分で定義したものを抽出
    dic=dict(filter(lambda item : inspect.getmodule(item[1]) is None ,dic.items())) #importした関数などは排除
    ...
    def global_check(locals:List[str],parent_name:str,parent_type_japanese:str,variable_type_japanese:str):
        """グローバル変数と同名のローカル変数がないかチェック"""
        nonlocal global_vars,builtins
        for l in locals:
            if l in global_vars:
                error=RedefinitionError(f"{parent_type_japanese}{parent_name}内の{variable_type_japanese}{l}はグローバル変数として定義されています。\nグローバル変数として使いたい場合はglobal宣言を行ってください。\nローカル変数として使いたい場合は別の名前を用いてください")
                error.local_name=l
                error.parent_name=parent_name
                raise error

    def builtin_check(locals:List[str],parent_name:str,parent_type_japanese:str,variable_type_japanese:str):
        """ビルトイン関数と同名のローカル変数がないかチェック"""
        nonlocal builtins
        for l in locals:
            if l in builtins:
                error=RedefinitionError(f"{parent_type_japanese}{parent_name}内の{variable_type_japanese}{l}はPythonに標準で定義されている変数です。別の名前に変えてください")
                error.local_name=l
                error.parent_name=parent_name
                raise error
    ...
    def class_check(cls:type):
        ...
        annot=list(cls.__annotations__.keys()) if hasattr(cls,"__annotations__") else []
        locals=list(filter(lambda x:not re.match("__.*__",x),list(members.keys())+annot)) 
    ...
    for v in dic.values():
        if type(v) is FunctionType: code_check(v.__code__)
        if type(v) is type : class_check(v)
