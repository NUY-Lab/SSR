...
class LinkamT95SerialIO:
    ...
    def connect(self, COMPORT: str) -> None:
        ...
        try:
            ...
            raise LinkamT95Error(
                f"{COMPORT}に繋がりません。\nデバイスマネージャーなどを調べてLinkamの正しいCOMポート番号を'COM(数字)'の形式でLinkamT95ManualContollerに入力してください"
            )
    ...
    def write(self, command: str) -> None:
        ...
        command:str
            コマンド
        """
        command = command + "\r"  # Carriage Returnを追加
    ...
    def query(self, command: str) -> bytes:
        ...
        """
        self.write(command)
        ans = self.ser.read_until(b"\r")  # \rを最後尾に含む応答が返ってくるまで待つ
        return ans
...
class LinkamT95IO:
    ...
    def set_lnp_speed(self, lnp_speed: int) -> None:
        ...
        if lnp_speed < 0:
            ...
            lnp_char = chr(lnp_speed + ord("0"))  # 数字を文字コードとして文字へ変換
    ...
    def read_status(self) -> Tuple[State, float, int]:
        ...
        state = self._read_statusbyte(ans[0])  # 測定状態(State)取得
        self._check_Error(ans[1])  # エラーチェック
        pump_speed = self._read_pump_statusbyte(ans[2])
        temperature = (
            int.from_bytes(ans[6:10], "big", signed=True) / 10.0
    ...
    def _read_pump_statusbyte(self, intdata: int) -> int:
        ...
        pump_speed = intdata - 0x80  # なぜかガス速度+0x80で返ってくるので引き算
        pump_speed = math.ceil(pump_speed / 3 * 10)  # 0～30で帰ってくるデータをタッチパネルと同じ0～100に変換する
